...
p6<-ggplot(data=cd, aes(x=factor(format(Death.Date, '%Y')),fill=Manner.Death))+geom_bar(position = "dodge")+scale_fill_brewer()+guides(fill= guide_legend(title = "Manner of Death"))+xlab("Year")+facet_wrap(~Custody.Type)+theme_dark()
p6
```  
...
```{r,warning=FALSE,message=FALSE,echo=FALSE,fig.align='center',fig.height=4,fig.width=7}
p4<-ggplot(data= cdt1, aes(x=Sex,y=Age,fill=Ethnicity))+geom_boxplot()+coord_flip()+scale_fill_brewer(palette = "Paired")+theme_dark()
p4
```  
